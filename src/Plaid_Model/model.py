...
def ibvmodel(
    jsonbody, modelfilepath="src/Plaid_Model/IBV_V3_TestBed.pkl", scalerfilepath="src/Plaid_Model/standardscaler_V3.pkl"
):
    logger.info("Starting ibvmodel()")
    ...
    try:
        ...
        ## Step 3: Feature Generation for IBV: to get the input of IBV_Model
        ...
        f13 = (
            f13.groupby("account_id")
            .mean()["MONTHS_WITH_EMPLOYMENT_RATE"]
        ...
        ibv_modelinput["IBVScore"] = 1000 - np.round(clf_ibv.predict_proba(standardized_data)[0][1] * 1000, 0).astype(
            int
        )
        ...
        logger.info(f"There was an error in step {stepname} executing the Plaid_model.")
        logger.exception(e)
        try:
